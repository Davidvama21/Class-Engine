...
D3D12_GPU_VIRTUAL_ADDRESS ModuleRingBuffer::allocBuffer(const void* buffer, size_t numBytes)
{
    ...
    /*
    _ASSERT_EXPR(numBytes < (totalSize - allocatedBytes), L"Out of memory, please allocate more memory at initialisation");
    if (tail < head)
    {
        size_t availableToEnd = size_t(totalSize - head);
        if (availableToEnd >= numBytes)
        {
            memcpy(pData + head, buffer, numBytes);
            ...
            bytesAllocatedInFrame[currentFrame] += numBytes;
            allocatedBytes += numBytes;
        ...
        else
        {
            ...
            head = 0;
            bytesAllocatedInFrame[currentFrame] += availableToEnd;
            allocatedBytes += availableToEnd;
        }
    }
    ...
    size_t available = tail == head && allocatedBytes == 0 ? totalSize : size_t(tail - head);
